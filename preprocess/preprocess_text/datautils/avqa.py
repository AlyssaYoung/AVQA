...
def multichoice_encoding_data(args, vocab, questions, video_names, video_ids, answers, ans_candidates, question_category):
    ...
    print('Writing', args.output_pt.format(args.dataset, args.mode))
    obj = {
    ...
    with open(args.output_pt.format(args.dataset, args.mode), 'wb') as f:
        pickle.dump(obj, f)
...
def process_questions_mulchoices(args):
    ...
    if args.mode in ['train']:
        ...
        with open(args.vocab_json.format(args.dataset), 'w') as f:
        ...
        print('Loading vocab')
        with open(args.vocab_json.format(args.dataset, args.method), 'r') as f:
            vocab = json.load(f)
        multichoice_encoding_data(args, vocab, questions, video_names, video_ids, answers, ans_candidates, question_category)
