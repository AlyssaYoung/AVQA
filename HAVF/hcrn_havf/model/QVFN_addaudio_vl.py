...
class HCRNNetwork(nn.Module):
    def __init__(self, vision_dim, audio_dim, module_dim, word_dim, k_max_frame_level, k_max_clip_level, spl_resolution, vocab, question_type, fuse_type):
        super(HCRNNetwork, self).__init__()

        self.question_type = question_type
        self.feature_aggregation = FeatureAggregation(module_dim)
        self.qvfusion_aggregation = QVFN(module_dim, fuse_type) # added
    ...
    def forward(self, ans_candidates, ans_candidates_len, video_appearance_feat, video_motion_feat, vl_audio_feat, cl_audio_feat, question,
        ...
        if self.question_type in ['frameqa', 'count', 'none']:
            ...
            question_embedding = self.linguistic_input_unit(question, question_len)
            visual_embedding = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding)
            vl_audio_feat_embedding = self.audio_feat_proj(vl_audio_feat)

            # q_visual_embedding = self.feature_aggregation(question_embedding, visual_embedding)
            qvfused_q_embedding = self.qvfusion_aggregation(question_embedding, visual_embedding, vl_audio_feat_embedding) #added
            ...
            # qvfused_a_embedding = self.qvfusion_aggregation(ans_candidates_embedding, visual_embedding[batch_agg], vl_audio_feat_embedding)
            out = self.output_unit(question_embedding[batch_agg], qvfused_q_embedding[batch_agg],
