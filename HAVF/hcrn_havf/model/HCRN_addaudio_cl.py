...
class InputUnitVisual(nn.Module):
    def __init__(self, k_max_frame_level, k_max_clip_level, spl_resolution, vision_dim, audio_dim, module_dim=512):
        ...
        self.clip_level_audio_cond = CRN(module_dim, k_max_frame_level, k_max_frame_level, gating=False, spl_resolution=spl_resolution)
        ...
        self.audio_sequence_encoder = nn.LSTM(audio_dim, module_dim, batch_first=True, bidirectional=False)
        ...
        self.video_level_audio_proj = nn.Linear(module_dim, module_dim)
    ...
    def forward(self, appearance_video_feat, motion_video_feat, question_embedding, cl_audio_feat):
        """
        Args:
            ...
            cl_audio_feat: [Tensor] (batch_size, num_clips , audio_dim)
        return:
        ...
        for i in range(appearance_video_feat.size(1)):
            ...
            clip_level_audio = cl_audio_feat[:, i, :] # (bz, 2048)
            clip_level_audio_proj = self.audio_feat_proj(clip_level_audio)
            ...
            # clip level CRNs
            clip_level_crn_audio = self.clip_level_audio_cond(torch.unbind(clip_level_appearance_proj, dim=1),
                                                                clip_level_audio_proj)
            # print(len(clip_level_crn_audio), clip_level_crn_audio[0].shape)
            clip_level_crn_question = self.clip_level_question_cond(clip_level_crn_audio, question_embedding_proj)
            # print(len(clip_level_crn_question), clip_level_crn_question[0].shape)
        ...
        _, (video_level_audio, _) = self.sequence_encoder(cl_audio_feat)
        video_level_audio = video_level_audio.transpose(0, 1)
        video_level_audio_feat_proj = self.video_level_audio_proj(video_level_audio)
        # video level CRNs
        video_level_crn_audio = self.video_level_audio_cond(clip_level_crn_outputs, video_level_audio_feat_proj)
        # print(len(video_level_crn_audio), video_level_crn_audio[0].shape)
        video_level_crn_question = self.video_level_question_cond(video_level_crn_audio, question_embedding_proj.unsqueeze(1))

        # print(len(video_level_crn_question), video_level_crn_question[0].shape)
        ...
        # print(video_level_crn_output.shape)
...
class HCRNNetwork(nn.Module):
    def __init__(self, vision_dim, audio_dim, module_dim, word_dim, k_max_frame_level, k_max_clip_level, spl_resolution, vocab, question_type):
        ...
        if self.question_type in ['action', 'transition']:
            ...
            self.visual_input_unit = InputUnitVisual(k_max_frame_level=k_max_frame_level, k_max_clip_level=k_max_clip_level, spl_resolution=spl_resolution, vision_dim=vision_dim, audio_dim=audio_dim, module_dim=module_dim)
    ...
    def forward(self, ans_candidates, ans_candidates_len, video_appearance_feat, video_motion_feat, vl_audio_feat, cl_audio_feat, question,
        ...
        Args:
            ...
            cl_audio_feat: [Tensor](batch_size, num_clips, audio_inp_dim)
            question: [Tensor] (batch_size, max_question_length)
            question_len: [Tensor] (batch_size)
        return:
            logits.
        ...
        if self.question_type in ['frameqa', 'count', 'none']:
            ...
            question_embedding = self.linguistic_input_unit(question, question_len)
            visual_embedding = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding, cl_audio_feat)
