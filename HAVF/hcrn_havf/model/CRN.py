...
class CRN_mul(Module):
    def __init__(self, module_dim, num_objects, max_subset_size, gating=False, spl_resolution=1, fuse_type='mul'):
        super(CRN_mul, self).__init__()
        self.module_dim = module_dim
        self.gating = gating
        self.fuse_type = fuse_type
        self.cond_feat_proj = nn.Linear(module_dim, module_dim)
            ...
            for id_choice, idx in enumerate(idx_relations_randomsample):
                ...
                if len(g_feat.size()) == 2:
                    if self.fuse_type == 'concat':
                        ...
                        cond_feat = self.cond_feat_proj(cond_feat)
                        h_feat = torch.mul(g_feat, cond_feat)
                        # h_feat = torch.mul(g_feat, F.sigmoid(cond_feat)) # To be tested
                elif len(g_feat.size()) == 3:
                    cond_feat_repeat = cond_feat.repeat(1, g_feat.size(1), 1)
                    if self.fuse_type == 'concat':
                        h_feat = torch.cat((g_feat, cond_feat_repeat), dim=-1)
                    elif self.fuse_type == 'mul':
                        cond_feat_repeat = self.cond_feat_proj(cond_feat_repeat)
                        h_feat = torch.mul(g_feat, cond_feat_repeat)
                if self.gating:
                    if self.fuse_type == 'concat':
                        h_feat = self.activation(self.k_objects_fusion[scaleID](h_feat)) * torch.sigmoid(
                            self.gate_k_objects_fusion[scaleID](h_feat))
