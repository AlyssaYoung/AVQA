...
class InputUnitAudio(nn.Module):
    def __init__(self, audio_dim, module_dim=512, fuse_type='gating'):
        ...
        self.mutan_fusion = MutanFusion(input_dim=module_dim, out_dim=module_dim)
    ...
    def forward(self, vl_audio_feat, question_embedding):
        ...
        elif self.fuse_type == 'mutan':
            h_feat = self.mutan_fusion(question_embedding_proj, audio_feat_proj)
...
class InputUnitVisual(nn.Module):
    ...
    def forward(self, appearance_video_feat, motion_video_feat, question_embedding, vl_audio_feat, cl_audio_feat):
        """
        Args:
...
class OutputUnitMultiChoices(nn.Module):
    def __init__(self, module_dim=512):
        ...
        self.audio_proj = nn.Linear(module_dim, module_dim)
...
class HCRNNetwork(nn.Module):
    def __init__(self, vision_dim, audio_dim, module_dim, word_dim, k_max_frame_level, k_max_clip_level, spl_resolution, vocab, question_type):
        ...
        if self.question_type in ['action', 'transition']:
            ...
            self.audio_input_unit = InputUnitAudio(audio_dim=audio_dim, module_dim=module_dim, fuse_type='mutan')
    ...
    def forward(self, ans_candidates, ans_candidates_len, video_appearance_feat, video_motion_feat, vl_audio_feat, cl_audio_feat, question,
        ...
        if self.question_type in ['frameqa', 'count', 'none']:
            ...
            question_embedding = self.linguistic_input_unit(question, question_len)
            visual_embedding = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding, vl_audio_feat, cl_audio_feat)
            ...
            out = self.output_unit(question_embedding[batch_agg], q_visual_embedding[batch_agg],
                                   ...
                                   a_visual_embedding, audio_embedding[batch_agg])
