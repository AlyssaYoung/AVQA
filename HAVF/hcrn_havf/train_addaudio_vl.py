...
import model.HCRN_addaudio_vl_multilevel as HCRN_multilevel
import model.HCRN_addaudio_vl_multilevel_onlyvcrn as HCRN_multilevel_middleconcat_onlyvcrn
import model.HCRN_addaudio_vl_multilevel_late as HCRN_multilevel_late
...
def train(cfg):
    ...
    if cfg.ensemble:
        ...
        model_early = HCRN_multilevel.HCRNNetwork(**model_early_kwargs).to(device)
        ckpt_early_path = '/DATA/DATANAS1/yangpinci/VGGSound_QA/hcrn-block/exp4_hcrn_addaudio_vl_multilevel_earlyconcat/ckpt/model.pt'
        ckpt_early = torch.load(ckpt_early_path, map_location=lambda storage, loc: storage)
        model_early.load_state_dict(ckpt_early['state_dict'])
        model_middle = HCRN_multilevel_middleconcat_onlyvcrn.HCRNNetwork(**model_middle_kwargs).to(device)
        ckpt_middle_path = '/DATA/DATANAS1/yangpinci/VGGSound_QA/hcrn-block/exp5_hcrn_addaudio_vl_multilevel_middleconcat_onlyvcrn/ckpt/model.pt'
        ckpt_middle = torch.load(ckpt_middle_path, map_location=lambda storage, loc: storage)
        model_middle.load_state_dict(ckpt_middle['state_dict'])
        model_late = HCRN_multilevel_late.HCRNNetwork(**model_late_kwargs).to(device)
        ckpt_late_path = '/DATA/DATANAS1/yangpinci/VGGSound_QA/hcrn-block/exp5_hcrn_addaudio_vl_multilevel_latemul/ckpt/model.pt'
        ckpt_late = torch.load(ckpt_late_path, map_location=lambda storage, loc: storage)
        model_late.load_state_dict(ckpt_late['state_dict'])
        # model = EnsembleModel(model_early, model_middle, model_late)
        model = EnsembleModel(model_early, model_late)
        model.train(False)
...
def main():
    ...
    parser.add_argument('--useAudio', default=True, type=bool)
